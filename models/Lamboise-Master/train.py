...
def train_model(model,
    ...
    if reload:
        model.load_state_dict(torch.load('/content/drive/MyDrive/capstone/Lamboise-Master/Weights/last.pth',map_location=torch.device(device)))
    ...
    if reload:
        try:
            prev_loss = np.loadtxt('/content/drive/MyDrive/capstone/Lamboise-Master/Loss/last.pth')
            ...
            prev_metrics = np.loadtxt('/content/drive/MyDrive/capstone/Lamboise-Master/Loss/last_metrics.pth')
    ...
    for epochs in range(0, num_epochs):
        ...
        # TEST
        test_metrics = evaluation(model, test_dataset, device, save_mask=False, plot_roc=True, print_metric=False)
        ...
        train_.append(float(loss_train))
        test_.append(float(loss_test))
    ...
    if save_model:
        placeholder_file('/content/drive/MyDrive/capstone/Lamboise-Master/Weights/last.pth')
        torch.save(model.state_dict(), '/content/drive/MyDrive/capstone/Lamboise-Master/Weights/last.pth')

        placeholder_file('/content/drive/MyDrive/capstone/Lamboise-Master/Weights/' + current_datetime + "-" + str(prev_epochs+num_epochs) + '.pth')
        torch.save(model.state_dict(), '/content/drive/MyDrive/capstone/Lamboise-Master/Weights/' + current_datetime + "-" + str(prev_epochs+num_epochs) + '.pth')
        ...
        placeholder_file(
            '/content/drive/MyDrive/capstone/Lamboise-Master/Loss/' + 'learning_' + str(learning_rate) + '_epoch_' + str(num_epochs) + '_time_' + current_datetime + '.pth')
        np.savetxt(
            '/content/drive/MyDrive/capstone/Lamboise-Master/Loss/' + 'learning_' + str(learning_rate) + '_epoch_' + str(num_epochs) + '_time_' + current_datetime + '.pth',
            loss_to_save)
        placeholder_file('/content/drive/MyDrive/capstone/Lamboise-Master/Loss/last.pth')
        np.savetxt('/content/drive/MyDrive/capstone/Lamboise-Master/Loss/last.pth', loss_to_save)
        ...
        np.savetxt('/content/drive/MyDrive/capstone/Lamboise-Master/Loss/last_metrics.pth', metrics_to_save)

    # Plot train and test losses and metrics
    plt.clf()
    x = [int(i) for i in range(0, len(losses_train)-1)]
    plt.plot(x, train_, label='Train Loss')
    plt.plot(x, test_, label='Test Loss')
    ...
    #plt.plot(metrics_idx, [float(1-auc_) for auc_ in auc],label='1-AUC')
    #plt.plot(metrics_idx, [1-auc_ for auc_ in auc],label='1 - AUC (AUC = '+ str(round(float(auc[len(auc)-1]), 3)) +')')
    #plt.plot(metrics_idx, [float(1-f1) for f1 in f1_score],label='1-F1')
    #plt.plot(metrics_idx, [1-f1 for f1 in f1_score],label='1 - F1 (F1 = '+ str(round(float(f1_score[len(f1_score)-1]), 3)) +')')
    plt.legend()
    plt.ylim(bottom=0, top=1)
    plt.xlabel("Epochs")
    plt.ylabel("Metric")
    plt.savefig("/content/drive/MyDrive/capstone/Lamboise-Master/Loss.png")
    plt.show()
    plt.close("/content/drive/MyDrive/capstone/Lamboise-Master/Loss.png")
    ...
    plt.xlabel("Epochs")
    plt.ylabel("Metric")
    plt.savefig("/content/drive/MyDrive/capstone/Lamboise-Master/1-auc.png")
    plt.show()
    plt.close("/content/drive/MyDrive/capstone/Lamboise-Master/1-auc.png")
    # FINI !
